...
export function parallax() {
  ...
  for (let i = 0; i < ellipses.length; i++) {
    let el = window.getComputedStyle(
      document.getElementById(`ellipse${i + 1}`) as HTMLDivElement
    );
    let elWrite: HTMLElement | null = document.getElementById(
      `ellipse${i + 1}`
    );

    if (el != undefined && elWrite != undefined) {
      let styleLeft: number = +el.left.substring(0, el.left.length - 2);
      let styleTop: number = +el.top.substring(0, el.top.length - 2);
      // elWrite.style.left =  `${random(styleLeft) + styleLeft }px`;
      //  elWrite.style.top =  `${random( styleTop) + styleTop }px`;
      if (coorY <= styleTop) {
        elWrite.style.top = `${styleTop - random(coorX)}px`;
      } else {
        elWrite.style.top = `${styleTop + random(coorX)}px`;
      }

      if (coorX <= styleLeft) {
        elWrite.style.left = `${styleLeft - random(coorY)}px`;
      } else {
        elWrite.style.left = `${styleLeft + random(coorY)}px`;
      }
    }
